...
# ================= 1. 页面全局配置 =================
st.set_page_config(page_title="Phenotype-Aware CVD Stratification")
...
def draw_academic_risk_bar(model_name, subtitle_html, probability, is_dual_track=False):
    ...
    if prob_pct >= 50:
        bar_color, status_text = "#9F1239", "≥50% display category"
    elif prob_pct >= 30:
        bar_color, status_text = "#B45309", "30–<50% display category"
    else:
        bar_color, status_text = "#0D9488", "<30% display category"
...
st.markdown('<div class="main-header">Phenotype-Aware CVD Stratification: Gatekeeper-Guided Dual-Track System</div>',
            unsafe_allow_html=True)
st.markdown(
    '<div class="sub-header">Research prototype demonstrating gatekeeper-guided routing, model-estimated probabilities, and probability reassignment.</div>',
    unsafe_allow_html=True)

st.caption(
    "Research use only. The gatekeeper does not replace UACR-based ODKD diagnosis; "
    "the displayed CVD probabilities refer to prevalent or concurrent CVD, not future incident events. "
    "The 30% and 50% boundaries are visualization categories, not treatment thresholds."
)

if gatekeeper is not None:
    st.sidebar.markdown("### 📋 Clinical Input")
    with st.sidebar.expander("👤 Basic & Metabolic", expanded=True):
        age = st.number_input("Age", 18.0, 100.0, 55.0)
        hba1c = st.number_input("HbA1c", 4.0, 15.0, 9.9)
        ua = st.number_input("SUA", 100.0, 800.0, 405.0)
        non_hdl = st.number_input("Non-HDL-C", 0.0, 10.0, 3.04)

    with st.sidebar.expander("🩸 Complete Blood Count", expanded=True):
        rdw = st.number_input("RDW", 10.0, 25.0, 12.6)
        neu = st.number_input("NEU#", 0.0, 20.0, 9.36)
        lym = st.number_input("LYM#", 0.0, 10.0, 1.09)
        mon = st.number_input("MON#", 0.0, 5.0, 1.14)
        plt = st.number_input("PLT", 50.0, 500.0, 200.0)
        mcv = st.number_input("MCV", 60.0, 120.0, 87.5)

    with st.sidebar.expander("🧪 Organ Function & Ions", expanded=True):
        bun = st.number_input("BUN", 0.0, 50.0, 8.47)
        scr = st.number_input("SCr", 30.0, 300.0, 124.0)
        alt = st.number_input("ALT", 0.0, 200.0, 27.0)
        ag_ratio = st.number_input("A/G Ratio", 0.5, 3.0, 1.394531, step=0.000001, format="%.6f")
        cl = st.number_input("Cl", 80.0, 120.0, 98.2)
        k = st.number_input("K", 2.0, 7.0, 4.39)
    ...
    if st.sidebar.button("🔍 Run Prediction", use_container_width=True, type="primary"):
        ...
        gatekeeper_probability = gatekeeper.predict_proba(df_gate)[0][1]
        is_odkd_positive = gatekeeper_probability >= 0.532
        ...
        if is_odkd_positive:
            ...
            shield_html = f"""<div style="display: flex; align-items: center; background-color: #F8FAFC; padding: 8px 15px; border-radius: 6px; border-left: 6px solid #9F1239; margin-bottom: 8px;">
...
<p style="margin: 0; color: #475569; font-size: 1.05rem; font-weight: 600;">Gatekeeper probability: {gatekeeper_probability:.3f} (routing cutoff 0.532) ➔ Routed to <b>Track A</b></p></div></div>"""
        else:
            dual_track_risk = track_b.predict_proba(df_track_b)[0][1]
            shield_html = f"""<div style="display: flex; align-items: center; background-color: #F8FAFC; padding: 8px 15px; border-radius: 6px; border-left: 6px solid #14B8A6; margin-bottom: 8px;">
<span style="font-size: 1.8rem; margin-right: 15px;">🛡️</span>
<div><h5 style="margin: 0; color: #1E293B; font-weight: 800; font-size: 1.15rem;">Gatekeeper Triage Active</h5>
<p style="margin: 0; color: #475569; font-size: 1.05rem; font-weight: 600;">Gatekeeper probability: {gatekeeper_probability:.3f} (routing cutoff 0.532) ➔ Routed to <b>Track B</b></p></div></div>"""
...
<h3 style="color: #0F766E; margin: 0 0 4px 0; font-size: 1.35rem;">🟢 Downward Probability Reassignment</h3>
<p style="margin: 0; color: #115E59; font-size: 1.1rem; font-weight: 600; line-height: 1.3;">The dual-track estimate crossed from ≥50% to &lt;50% relative to the global model. The 50% boundary is a web-display category, not a treatment threshold.</p>
</div>"""
...
<h3 style="color: #881337; margin: 0 0 4px 0; font-size: 1.35rem;">🔴 Upward Probability Reassignment</h3>
<p style="margin: 0; color: #4C0519; font-size: 1.1rem; font-weight: 600; line-height: 1.3;">The dual-track estimate crossed from &lt;50% to ≥50% relative to the global model. The 50% boundary is a web-display category, not a treatment threshold.</p>
</div>"""
...
<h3 style="color: #374151; margin: 0 0 4px 0; font-size: 1.35rem;">⚖️ No Cross-Category Reassignment</h3>
<p style="margin: 0; color: #4B5563; font-size: 1.1rem; font-weight: 600; line-height: 1.3;">Both estimates remained on the same side of the 50% web-display boundary. This does not imply clinical consensus or a treatment recommendation.</p>
</div>"""
