...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = './output/2_Model_Training_and_Evaluation/KNN/'
    ...
    print("Initializing KNN hyperparameter tuning via Grid Search...")

    # Data preprocessing pipeline: Imputation -> Scaling -> KNN
    pipeline = Pipeline([
    ...
    params_df['Parameter'] = params_df['Parameter'].str.replace('knn__', '')

    output_excel_path = os.path.join(OUTPUT_DIR, 'KNN_Evaluation_Metrics.xlsx')
    try:
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'KNN_ROC_Curve.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'KNN_PR_Curve.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'KNN_Confusion_Matrix.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
