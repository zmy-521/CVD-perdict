...
# Algorithm: Artificial Neural Network (ANN / MLP)
# ==============================================================================
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = './output/2_Model_Training_and_Evaluation/ANN/'
    ...
    # NOTE: Slicing starts from column 5 based on the specific dataset structure
    X = data.iloc[:, 5:]
    y = data.iloc[:, 0]
    ...
    print("Initializing ANN hyperparameter tuning via Grid Search...")

    # Data preprocessing pipeline: Imputation -> Scaling -> MLP
    pipeline = Pipeline([
    ...
    param_grid = {
        'mlp__hidden_layer_sizes': [(32,), (16, 8), (32, 16)],
    ...
    params_df['Parameter'] = params_df['Parameter'].str.replace('mlp__', '')

    output_excel_path = os.path.join(OUTPUT_DIR, 'ANN_Evaluation_Metrics.xlsx')
    try:
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'ANN_ROC_Curve.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'ANN_PR_Curve.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'ANN_Confusion_Matrix.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
