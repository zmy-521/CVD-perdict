...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = './output/2_Model_Training_and_Evaluation/RF/'
    ...
    # NOTE: Slicing starts from column 2 based on the specific dataset structure
    X = data.iloc[:, 2:]
    y = data.iloc[:, 0]
    ...
    print("Initializing Random Forest hyperparameter tuning via Grid Search...")

    # Data preprocessing pipeline: Imputation -> Random Forest
    # Note: scikit-learn's RandomForest requires imputation for missing values
    pipeline = Pipeline([
    ...
    params_df['Parameter'] = params_df['Parameter'].str.replace('rf__', '')

    output_excel_path = os.path.join(OUTPUT_DIR, 'RF_Evaluation_Metrics.xlsx')
    try:
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'RF_ROC_Curve.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'RF_PR_Curve.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'RF_Confusion_Matrix.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
