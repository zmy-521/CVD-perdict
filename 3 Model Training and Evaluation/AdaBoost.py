...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = './output/2_Model_Training_and_Evaluation/AdaBoost/'
    ...
    print("Initializing AdaBoost hyperparameter tuning via Grid Search...")

    # Data imputation integrated into the modeling pipeline to prevent data leakage
    pipeline = Pipeline([
    ...
    params_df['Parameter'] = params_df['Parameter'].str.replace('ada__', '')

    output_excel_path = os.path.join(OUTPUT_DIR, 'AdaBoost_Evaluation_Metrics.xlsx')
    try:
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'AdaBoost_ROC_Curve.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'AdaBoost_PR_Curve.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'AdaBoost_Confusion_Matrix.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
