...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = './output/2_Model_Training_and_Evaluation/DT/'
    ...
    print("Initializing Decision Tree hyperparameter tuning via Grid Search...")

    # Data preprocessing pipeline: Imputation -> Decision Tree
    ...
    params_df['Parameter'] = params_df['Parameter'].str.replace('dt__', '')

    output_excel_path = os.path.join(OUTPUT_DIR, 'DT_Evaluation_Metrics.xlsx')
    try:
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'DT_ROC_Curve.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'DT_PR_Curve.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'DT_Confusion_Matrix.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
