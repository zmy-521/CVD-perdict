...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = './output/2_Model_Training_and_Evaluation/LR/'
    ...
    print("Initializing Logistic Regression hyperparameter tuning via Grid Search...")

    # Data preprocessing pipeline: Imputation -> Scaling -> Logistic Regression
    pipeline = Pipeline([
    ...
    params_df['Parameter'] = params_df['Parameter'].str.replace('lr__', '')

    output_excel_path = os.path.join(OUTPUT_DIR, 'LR_Evaluation_Metrics.xlsx')
    try:
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'LR_ROC_Curve.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'LR_PR_Curve.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, 'LR_Confusion_Matrix.pdf'), format='pdf', bbox_inches='tight')
    plt.close()
